#!/usr/bin/env python3
"""
Comprehensive OpenMM membrane MD analysis script.
Adapted from GROMACS version for cross-validation.

Usage:
    cd exps/exp-F/md/membrane_rep1 && python comprehensive_analysis_openmm.py
...
OMM_DIR = f"{REPO}/exps/exp-F/md/membrane_rep1"
OUTDIR = f"{OMM_DIR}/analysis_openmm"

# Full system (for membrane analysis)
PRMTOP = f"{REPO}/exps/exp-F/membrane_build/system_final.prmtop"
DCD = f"{OMM_DIR}/prod_v2.dcd"
OMM_LOG = f"{REPO}/exps/exp-F/md/membrane_v2_200ns.log"

# Protein trajectory (OpenMM DCD is unwrapped, no PBC correction needed)
# Use full system for both full-system and protein analysis
PRMTOP_PROT = PRMTOP
DCD_PROT = DCD
...
log_file = open(f"{OUTDIR}/analysis_openmm.log", "w")
...
log("="*70)
log("COMPREHENSIVE OPENMM MEMBRANE MD ANALYSIS")
log("="*70)
...
u_full = mda.Universe(PRMTOP, DCD)
n_frames_full = len(u_full.trajectory)
log(f"  Full system:   {len(u_full.atoms):6d} atoms, {n_frames_full:5d} frames")

u_prot = mda.Universe(PRMTOP_PROT, DCD_PROT)
n_frames_prot = len(u_prot.trajectory)
log(f"  Protein whole: {len(u_prot.atoms):6d} atoms, {n_frames_prot:5d} frames")

# Reference structures (first frame of trajectory)
ref_full = mda.Universe(PRMTOP, DCD)
ref_full.trajectory[0]
ref_prot = mda.Universe(PRMTOP_PROT, DCD_PROT)
ref_prot.trajectory[0]
...
# =============================================================================
# 2. ENERGY (from OpenMM log CSV)
# =============================================================================
log("\n[2] Energy analysis...")
try:
    # OpenMM log format: step,time(ps),potential,kinetic,total,temperature,volume,density,speed
    df_energy = pd.read_csv(OMM_LOG, header=None,
                            names=['Step','Time','Potential','Kinetic','Total','Temperature','Volume','Density','Speed'])
    # Filter out non-data lines (e.g. "Checkpoint saved at step ...") and convert to numeric
    df_energy['Step'] = pd.to_numeric(df_energy['Step'], errors='coerce')
    df_energy = df_energy[df_energy['Step'].notna()]
    for col in ['Time','Potential','Kinetic','Total','Temperature','Volume','Density','Speed']:
        df_energy[col] = pd.to_numeric(df_energy[col], errors='coerce')
    avail = [c for c in ['Potential','Temperature','Volume','Density'] if c in df_energy.columns]
    ...
    import traceback
    log(f"  Energy traceback: {traceback.format_exc()}")
    energy_ok = False
...
# Compute area/lipid from trajectory box dimensions (works for both GROMACS and OpenMM)
try:
    box_xy = []
    mem_times = []
    for ts in u_full.trajectory[::STEP]:
        ...
        if dims is not None and len(dims) >= 2:
            # dimensions: [lx, ly, lz, alpha, beta, gamma] in Angstrom
            box_xy.append(dims[0] * dims[1])
            mem_times.append(ts.time / 1000.0)
    box_xy = np.array(box_xy)
    mem_times = np.array(mem_times)
    area_per_lipid = box_xy / (n_lipids / 2)
    
    df_mem = pd.DataFrame({
        'Time_ns': mem_times,
        'Area_per_lipid_A2': area_per_lipid,
        'Membrane_thickness_A': np.interp(mem_times, times_pos, mem_thickness),
    })
    ...
    log(f"  Membrane properties failed: {e}")
    import traceback
    log(f"  Membrane traceback: {traceback.format_exc()}")
...
report = f"""================================================================================
OPENMM MEMBRANE MD ANALYSIS SUMMARY
================================================================================
Trajectory: {DCD}
Protein trajectory: {DCD_PROT}
...
if energy_ok and 'Volume' in df_energy.columns:
    report += f"""
--- MEMBRANE ---
