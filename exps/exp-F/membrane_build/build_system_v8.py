...
# Read protein - remove LNK hydrogens and all hydrogens from GLY 101 (chain P)
protein_atoms = []
    ...
    for line in f:
        if line.startswith('ATOM') or line.startswith('HETATM'):
            ...
            if resname == 'LNK' and atomname.startswith('H'):
                continue
            if chain == 'P' and resnum == 101 and resname == 'GLY' and atomname.startswith('H'):
                continue
...
print(f"Protein atoms (LNK H + GLY H stripped): {len(protein_atoms)}")
...
# Write protein PDB with TER between chains
atom_counter = 1
current_chain = None
with open('protein_final.pdb', 'w') as out:
    for atom in protein_atoms:
        ...
        # Fix atom name alignment for PDB standard (right-align in cols 13-16 for 1-char elements)
        name = atom['atomname']
        if len(name) == 1:
            name = f"   {name}"
        elif len(name) == 2:
            ...
            name = f"{name:4s}"
        out.write(f"ATOM  {format_atomnum(atom_counter)} {name} {atom['resname']:>3s} {atom['chain']:>1s}{format_resnum(atom['resnum'])}    {atom['x']:>8.3f}{atom['y']:>8.3f}{atom['z']:>8.3f}  1.00  0.00           {atom['element']:>2s}\n")
        atom_counter += 1
    out.write("TER\n")
    out.write("END\n")
        ...
        if map_key in mapping:
            ...
            for atom in converted:
                ...
                if len(name) == 1:
                    ...
                    name = f"{name:4s}"
                out.write(f"ATOM  {format_atomnum(atom_counter)} {name} {atom['resname']:>3s} {chain:>1s}{format_resnum(resnum)}    {atom['x']:>8.3f}{atom['y']:>8.3f}{atom['z']:>8.3f}  1.00  0.00           {atom['element']:>2s}\n")
                atom_counter += 1
            out.write("TER\n")
        else:
            resnum = 1
...
# Write waters PDBs - split to keep each under 99999 atoms
MAX_ATOMS_PER_FILE = 99999
water_files = []
atom_counter = 1
file_idx = 1
wchain_idx = 0
...
for i, mol in enumerate(water_mols):
    if atom_counter > MAX_ATOMS_PER_FILE:
        out.write("END\n")
        out.close()
        print(f"Wrote {water_files[-1]} with {atom_counter-1} atoms")
        atom_counter = 1
        file_idx += 1
        out = None
    
    if out is None:
        fname = f'water_final_{file_idx}.pdb'
        water_files.append(fname)
        out = open(fname, 'w')
    ...
    for atom in mol:
        ...
        if len(name) == 1:
            ...
            name = f"{name:4s}"
        out.write(f"ATOM  {format_atomnum(atom_counter)} {name} {'WAT':>3s} {chain:>1s}{format_resnum(wresnum)}    {atom['x']:>8.3f}{atom['y']:>8.3f}{atom['z']:>8.3f}  1.00  0.00           {atom['element']:>2s}\n")
        atom_counter += 1
    wresnum += 1
...
# Write ions PDB - fix atom names for tleap compatibility
atom_counter = 1
ichain_idx = 0
iresnum = 1
ion_files = ['ions_final.pdb']
with open('ions_final.pdb', 'w') as out:
    for i, mol in enumerate(ion_mols):
        ...
        for atom in mol:
            ...
            if resname == 'Cl-' and atomname == 'Cl':
                atomname = 'Cl-'
            elif resname == 'K+' and atomname == 'K':
                atomname = 'K+'
            name = atomname
            if len(name) == 1:
                name = f"   {name}"
                ...
                name = f"{name:4s}"
            out.write(f"ATOM  {format_atomnum(atom_counter)} {name} {resname:>3s} {chain:>1s}{format_resnum(iresnum)}    {atom['x']:>8.3f}{atom['y']:>8.3f}{atom['z']:>8.3f}  1.00  0.00           {atom['element']:>2s}\n")
            atom_counter += 1
        iresnum += 1
    out.write("END\n")

print(f"Wrote ions_final.pdb with {atom_counter-1} atoms")
print(f"Water files: {water_files}")
