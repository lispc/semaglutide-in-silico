...
with open('protein.pdb') as f:
    for line in f:
        if line.startswith('ATOM') or line.startswith('HETATM'):
            ...
            # Skip all hydrogens from GLY 101 (chain P) to let tleap rebuild
...
for i, mol in enumerate(water_mols):
    ...
    for atom in mol:
        # Fix water atom names for tleap: OH2 -> O
        atomname = atom['atomname']
        if atomname == 'OH2':
            atomname = 'O'
        out.write(f"ATOM  {format_atomnum(atom_counter)} {atomname:4s} {'WAT':>3s}{chain:>1s}{format_resnum(wresnum)}    {atom['x']:>8.3f}{atom['y']:>8.3f}{atom['z']:>8.3f}  1.00  0.00           {atom['element']:>1s}\n")
        atom_counter += 1
    wresnum += 1
...
with open('ions_final.pdb', 'w') as out:
    for i, mol in enumerate(ion_mols):
        ...
        for atom in mol:
            # Fix atom names for tleap compatibility
            atomname = atom['atomname']
            ...
            if resname == 'Cl-' and atomname == 'Cl':
                ...
                atomname = 'K+'
            out.write(f"ATOM  {format_atomnum(atom_counter)} {atomname:4s} {resname:>3s}{chain:>1s}{format_resnum(iresnum)}    {atom['x']:>8.3f}{atom['y']:>8.3f}{atom['z']:>8.3f}  1.00  0.00           {atom['element']:>1s}\n")
            atom_counter += 1
        iresnum += 1
    out.write("END\n")
