...
## 进展（2026-07-25，production + 驻留分区分析完成）

- **Pilot 20 ns ✅**：预锚定双盐桥在生产开始后 ~2 ns 松脱（linker 使 FA3 解离加速 ~100×），
  但尾羧基全程保持 HSA 表面接触（2.53 ± 0.14 Å，后 25%），肽-ECD 界面稳定——
  桥接构型是稳态。详见 `exp-log.md` 2026-07-21 条目
  （⚠️ 该条目中"肽-ECD 1.34 Å"的度量口径不明、无法复现；production 分析改用最小重原子距离，frame-0 = 2.92 Å，定性结论不变）
- **Production 3×100 ns ✅（2026-07-25）**：seeds 101/202/303，驻留分区分析见 `analysis/RESULTS.md`
- **远端羧基驻留分区（合并 n=3 帧占比）**：FA3 口袋 20.2% / HSA 表面 45.2% / 过渡区 29.0% / 水相 5.5%。
  三个 replica 三种命运：rep1 全程 FA3↔表面动态交换（346 次，不脱逃）；rep2 3.9 ns 脱锚转表面；rep3 2.5 ns 脱锚渐溶剂化
- **核心结论**：(1) FA3 是快速交换的驻留区而非静态锚点；(2) linker 两端尾端先松且松 ~2 个数量级
  （肽-ECD 界面 2.77±0.04 Å，300 ns 零脱钩）——受体端亲和力 ≫ HSA 端；(3) **近端 O38-ARG408 盐桥
  在装上 linker 后不形成**（成桥率 0%×3，构建几何的假象），HSA 上只有远端羧基一个有效锚点；
  (4) 桥接构型在 300 ns 总采样下仍是稳态，但单轨迹结论在本体系不可信（pilot 只代表 rep2/3 型）
