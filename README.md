...
| exp-G (HSA/receptor competition) | ✅ Complete (3 replicas + residence analysis) | First ternary system (HSA + semaglutide + ECD, ~142k atoms, FA3 pre-anchored). Residence partition over 3×100 ns: tail carboxyl splits between FA3 pocket 20% / HSA surface 45% / transition 29% / water 5% — FA3 is a fast-exchange residence region, not a static anchor (rep1 exchanged 346 times without escaping; rep2/rep3 undocked at 3.9/2.5 ns). The peptide-ECD interface never let go (2.77 Å, 300 ns) — the receptor end binds ~2 orders of magnitude tighter than the albumin end, and the proximal-carboxyl ARG408 bridge does not form once the linker is attached (0% occupancy, a build-geometry artifact) |
...
- **The full competition system is exp-G (complete, 3×100 ns).** exp-C lacks receptor; exp-D lacks HSA; exp-F lacks HSA. exp-G (HSA + peptide-linker-FA + ECD, FA3 pre-anchored) closes this gap: residence-partition analysis (2026-07-25) shows FA3 acts as a fast-exchange region while the peptide stays on the ECD — but 100 ns × 3 is still short for slow (re)binding kinetics, and replica fates diverge (1/3 stays exchanging, 2/3 undock), so quantitative partition coefficients carry large uncertainty.
